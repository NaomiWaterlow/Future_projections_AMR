################################################################
####### Creates model predictions for hierarchical models ######
####### Developers: N Waterlow & G Knight ###################### 
################################################################

# This script generates future predictions for hierarchical models (both plain and age models) 
# for various interventions. It uses parallel processing to speed up the prediction process 
# by splitting the data into chunks. The script iterates over multiple models and pathogen combinations,
# and saves the predictions for each model into individual RDS files. 
# The script ensures that non-negative integer values for incidence are maintained, and models are checked 
# for consistency between plain ("base") and age-specific versions.
...
for(run_model in c(1:length(plain_model_names))
  ...
  for(ivt in 1:nrow(interventions)){
    ...
    if(!file.exists(paste0("predictions/individual_predictions",name_run,"_",
                           plain_model_names[run_model], ".RDS"))){
